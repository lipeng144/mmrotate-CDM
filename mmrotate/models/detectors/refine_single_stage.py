...
class RefineSingleStageDetector(BaseDetector):
    ...
    def loss(self, batch_inputs: Tensor,
        ...
        """
        x = self.extract_feat(batch_inputs)  #[batch_size,5]个不同大小的特征图

        losses = dict()
        outs = self.bbox_head_init(x)  #2*5*2*通道数*w*h 两个分支cls和reg
        ...
        rois = self.bbox_head_init.filter_bboxes(*outs)  #每一个锚点对应的九个先验框，取类别置信度最大的锚框（9取1），之后根据先验框和预测得到每一种级别的特征图预测框[2*5*不同层次的特征图的框的数量*5]
