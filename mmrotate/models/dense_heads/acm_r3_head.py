    ...
    def filter_bboxes(self, cls_scores: List[Tensor],
        ...
        for lvl in range(num_levels):
            ...
            best_pred = bbox_pred.gather(
                dim=-2, index=best_ind).squeeze(dim=-2) #[num_imgs,num_level,5]

            anchors = anchors.reshape(-1, self.num_anchors, 5).tensor  #[num_level,self.num_anchors,5]
...
@MODELS.register_module()
class R3RefineHead(CDAngleBranchRetinaHead):
    r"""An anchor-based head used in `R3Det
    <https://arxiv.org/pdf/1908.05612.pdf>`_.
    ...
    def loss_by_feat(self,
        ...
        return super(CDAngleBranchRetinaHead, self).loss_by_feat(
